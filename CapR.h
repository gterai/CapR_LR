...
class CapR{
 public:
  CapR(string input_file, string output_file, int maximal_span, Probing p){
    probing = p;
 ...
 private:
  ...
  double CalcStemProbabilityL(int i);
  void CalcStemProbability(vector<double> &stemL_probability, vector<double> &stemR_probability);
  //void CapR::dispMat(int x, int y);
  void calcPE(string seq, string sname, int shape_flag, int dms_flag);
  void zeroPE(string seq);
  void calcSHAPE_PE(string seq, string sname);
  void calcDMS_PE(string seq, string sname);
  double calcGamma(double shape, double x, double scale);
  double calcMixGamma(double value, vector<double> param);
